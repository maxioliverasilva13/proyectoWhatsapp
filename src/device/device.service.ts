...
export class DeviceService {
  ...
  async sendNotificationUser(userId: number, title: string, desc: string) {
    const usuario = await this.usuarioRepository.findOne({ where: { id: userId }, relations: ["dispositivo"] });
    ...
    try {
        ...
        throw new InternalServerErrorException("Error sending notification", error.message);
    }
}
